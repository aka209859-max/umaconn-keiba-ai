...
#### **HQSへの統合方針**
- **予想脚質はHQSスコア計算に必須**（的中精度向上のため）
- 各脚質の過去実績（的中率・補正回収率）を集計し、HQSスコアを算出
- 脚質別の統計データを `nar_hqs_index_stats` に保存
  - 例：「逃」の単勝的中率 25%、複勝的中率 60%、補正回収率 85%
  - 例：「追」の単勝的中率 15%、複勝的中率 45%、補正回収率 70%
- **展開マッチング**による補正
  - 「逃」が複数いる場合（ハイペース）→ 「逃」の信頼度を下げる
  - 「追」が多い場合（縦長馬群）→ 「追」の信頼度を上げる
...
| 指数名 | 重み | 根拠 |
|-------|------|------|
| **テン指数** | 20% | 序盤ポジション確保の重要性 |
| **位置指数** | 25% | レース展開の中核要素 |
| **上がり指数** | 25% | 決め手の有無を評価 |
| **ペース指数** | 15% | 補完的指標 |
| **予想脚質** | 15% | 的中精度向上に必須（展開マッチング） |
...
| `index_type` | VARCHAR(20) | 指数種別（'ten', 'position', 'agari', 'pace', 'ashishitsu'） |
| `index_value` | VARCHAR(10) | 指数値（数値: -100〜+100、脚質: '逃','先','差','追','好','自'） |
| `cnt_win` | INT | 単勝試行回数 |
...
| **予想脚質** | `nvd_se` | `computed_ashishitsu` または過去通過順位から計算 |
