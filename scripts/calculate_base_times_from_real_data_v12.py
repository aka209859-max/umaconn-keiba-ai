...
- 🔥 重要変更: すべての距離で zenhan_3f = soha_time - kohan_3f（統一計算）
"""
            ...
            for kyori in kyori_list:
                ...
                for row in rows:
                    try:
                        ...
                        # 🔥 統一計算: すべての距離で zenhan_3f = soha_time - kohan_3f
                ...
                # 距離に応じた説明を追加
                if kyori == 1200:
                    distance_note = "前半=前半600m"
                elif kyori < 1200:
                    distance_note = f"前半={kyori-600}m"
                else:
                    distance_note = f"前半={kyori-600}m"
                
                msg = f"  距離 {kyori:4d}m: 走破={median_soha_time:.1f}秒, 前半={median_zenhan_3f:.1f}秒（{distance_note}）, 後半3F={median_kohan_3f:.1f}秒 (サンプル数: {n:4d})"  # 🔥 修正: 統一計算
                print(msg)
                f.write(msg + "\n")
