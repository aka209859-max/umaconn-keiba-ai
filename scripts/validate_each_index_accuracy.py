...
【使用方法】
python scripts/validate_each_index_accuracy.py --data-path /path/to/data.csv --start-date 20231013 --end-date 20251231
...
def main():
    ...
    # コマンドライン引数のパース
    parser = argparse.ArgumentParser(description='NAR-SI3.0 Each Index Prediction Accuracy Validation')
    parser.add_argument('--data-path', type=str, default='/home/user/uploaded_files/data-1768047611955.csv',
                        help='Data file path')
    parser.add_argument('--start-date', type=str, default='20231013',
                        help='Start date in YYYYMMDD format')
    parser.add_argument('--end-date', type=str, default='20251231',
                        help='End date in YYYYMMDD format')
    parser.add_argument('--sample-rate', type=float, default=0.1,
                        help='Sampling rate, e.g., 0.1 for 10 percent')
    parser.add_argument('--output-dir', type=str, default='/home/user/webapp/nar-ai-yoso/data',
                        help='Output directory')
    
    args = parser.parse_args()
    
    print("=" * 100)
    print("📊 NAR-SI3.0 各指数ごとの予測精度検証（簡易版）")
    print("=" * 100)
    print(f"データパス: {args.data_path}")
    print(f"期間: {args.start_date} ～ {args.end_date}")
    print(f"サンプリング率: {int(args.sample_rate * 100)}%")
    print("=" * 100)
    
    # データ読み込み
    df = load_and_filter_data(args.data_path, args.start_date, args.end_date, args.sample_rate)
    ...
    # 結果保存
    output_path = os.path.join(args.output_dir, 'index_accuracy_comparison.csv')
    results_df.to_csv(output_path, index=False, encoding='utf-8-sig')
    print(f"\n💾 結果保存: {output_path}")
    
    # JSON形式でも保存
    json_output_path = os.path.join(args.output_dir, 'index_accuracy_comparison.json')
